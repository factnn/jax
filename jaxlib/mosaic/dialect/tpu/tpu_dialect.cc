...
namespace {
bool hasSharedMemorySpace(MemorySpaceAttr ms) {
...
FailureOr<bool> isGather(Operation& op, MemorySpaceAttr source_ms,
  ...
  if (source_ms == nullptr || target_ms == nullptr) {
    InFlightDiagnostic diag = op.emitError(
        "isGather requires non-null source and target memory space "
        "attributes.");
    if (source_ms == nullptr) {
      diag << " source memory space is null.";
    }
    if (target_ms == nullptr) {
      diag << " target memory space is null.";
    }
    return diag;
  }
  if (hasSharedMemorySpace(source_ms) &&
      target_ms.getValue() == MemorySpace::kVmem) {
    return true;
  }
  if (source_ms.getValue() == MemorySpace::kVmem &&
      hasSharedMemorySpace(target_ms)) {
    return false;
  }
