...
LogicalResult enqueue_dma_upgrade(Operation* op, int version, bool&) {
  ...
  if (version < 14) {
    ...
    auto core_type = TPUDialect::GetCoreTypeAttr(op);
    if (core_id_attr != nullptr && core_type.has_value() &&
        *core_type == CoreType::kScVectorSubcore) {
      // For version < 14, the only supported TPUs with SC vector subcores
      // have 16 subcores per core. Future TPUs should only ever see exports
      // with version >= 14.
      const int32_t num_subcores = 16;
      const int32_t subcore_id = core_id_attr.getInt() % num_subcores;
      op->setAttr("core_id", mlir::IntegerAttr::get(
